...
namespace ISNP151323_Unidad3
{
    internal class Conexion {
        ...
        public DataSet ObtenerDatos(){
            ...
            miComando.CommandText = "select * from cumple";
            miAdaptador.SelectCommand = miComando;
            miAdaptador.Fill(ds, "cumple");
            return ds;
        }
        ...
        public string AdministrarCumple(String[] cumple, String accion)
        {
            String sql = "";
            if (accion == "Nuevo")
            {
                sql = "INSERT INTO cumple (Registro, Nombre, Telefono, Fecha) VALUES ('" + cumple[1] + "', " +
                    "'" + cumple[2] + "', '" + cumple[3] + "', '" + cumple[4] + "')";
            } else if (accion == "Modificar")
            {
                sql = "UPDATE cumple SET Registro = '" + cumple[1] + "', " +
                    "Nombre = '" + cumple[2] + "', Telefono = '" + cumple[3] + "', Fecha = '" + cumple[4] + "'" +
                    "WHERE IdCumple = '" + cumple[0] + "'";
            } else if (accion == "Eliminar")
            {
                sql = "DELETE cumple WHERE IdCumple = '" + cumple[0] + "'";
            }
            return EjecutarSQL(sql);
        }
